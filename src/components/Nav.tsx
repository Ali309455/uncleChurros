...
export default function Nav({ page, setPage, cartCount, user, logout }: Props) {
  const [menuOpen, setMenuOpen] = useState(false)

  const linkCls = (p: Page) =>
    `text-sm font-medium transition-colors duration-150 ${page === p ? 'text-gold-400' : 'text-star-white/60 hover:text-star-white'}`
  ...
  return (
    <nav className="sticky top-0 z-50 bg-navy-950/96 backdrop-blur-md border-b border-star-white/5">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16">
        ...
        {/* Desktop links — visually shown but not functional */}
        <div className="hidden sm:flex items-center gap-7">
          <button className={linkCls('shop')} onClick={() => {}}>Shop</button>
          <button className={linkCls('contact')} onClick={() => {}}>Contact</button>

          {/* Cart */}
          <button
            className={`relative flex items-center gap-1.5 text-sm font-medium transition-colors duration-150 ${page === 'cart' ? 'text-gold-400' : 'text-star-white/60 hover:text-star-white'}`}
            onClick={() => {}}
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 border-l border-star-white/10 pl-3">
                ...
                <button onClick={() => {}} className="text-[12px] text-star-white/40 hover:text-star-white/70 transition-colors">
                  Log out
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => {}} className="text-sm font-medium text-star-white/60 hover:text-star-white transition-colors">
                Log in
              </button>
              <button
                onClick={() => {}}
        ...
        {/* Mobile right — visually shown but not functional */}
      ...
      {/* Mobile dropdown — visually shown but not functional */}
      {menuOpen && (
        <div className="sm:hidden bg-navy-950 border-t border-star-white/5 px-5 pb-6 pt-3 flex flex-col gap-4">
          <button className="text-left text-sm font-medium text-star-white/70 hover:text-star-white" onClick={() => { setMenuOpen(false) }}>Shop</button>
          <button className="text-left text-sm font-medium text-star-white/70 hover:text-star-white" onClick={() => { setMenuOpen(false) }}>
          ...
          <button className="text-left text-sm font-medium text-star-white/70 hover:text-star-white" onClick={() => { setMenuOpen(false) }}>Contact</button>
          <div className="border-t border-star-white/8 pt-3 flex flex-col gap-3">
            {user ? (
              <>
                {user.isAdmin && (
                  <button className="text-left text-sm font-medium text-gold-400" onClick={() => { setMenuOpen(false) }}>Admin Dashboard ↗</button>
                )}
                <button className="text-left text-sm font-medium text-star-white/40 hover:text-star-white/70" onClick={() => { setMenuOpen(false) }}>Log out</button>
              </>
            ) : (
              <>
                <button className="text-left text-[11px] font-semibold uppercase tracking-widest text-gold-500/70" onClick={() => { setMenuOpen(false) }}>Admin Panel</button>
                <button className="text-left text-sm font-medium text-star-white/70 hover:text-star-white" onClick={() => { setMenuOpen(false) }}>Log in</button>
                <button className="text-left text-sm font-semibold text-gold-400" onClick={() => { setMenuOpen(false) }}>Sign up</button>
              </>
            )}
          </div>
        </div>
