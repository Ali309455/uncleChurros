...
export default function LoginForm({ intendedAdmin = false }) {
  ...
  const doLogin = (isAdmin, name, emailAddr) => {
    setLoading(true)
    setTimeout(() => {
      login({ name, email: emailAddr, isAdmin })
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl"
          ...
          <div className="bg-navy-800/90 backdrop-blur-xl p-8">
            ...
            <p className="text-star-white/30 text-[12px] text-center mt-5">
              No account?{' '}
              <button onClick={() => router.push('/signup')} className="text-gold-500 hover:text-gold-400 font-medium transition-colors">
                Create one <ArrowIcon size={14} className="ml-1" />
              </button>
            </p>
          </div>
        </div>
