...
const BG_STARS = Array.from({ length: 55 }, (_, i) => {
  const s = (n: number) => { const x = Math.sin(i * 11.3 + n) * 10000; return x - Math.floor(x) }
  return { x: s(1) * 100, y: s(2) * 100, size: s(3) > 0.75 ? 2.2 : s(3) > 0.45 ? 1.4 : 0.8, opacity: 0.12 + s(4) * 0.32, gold: i % 13 === 0 }
})

export default function Signup({ setPage, login }: Props) {
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="bg-navy-800/80 backdrop-blur-xl border border-star-white/8 rounded-2xl p-8 shadow-2xl"
          ...
          <p className="text-star-white/30 text-[12px] text-center mt-5">
            ...
            <button onClick={() => setPage('login')} className="text-gold-500 hover:text-gold-400 font-medium transition-colors">
              Log in →
            </button>
          </p>
        </div>

        <button onClick={() => setPage('home')} className="mt-6 text-star-white/30 hover:text-star-white/60 text-[13px] transition-colors flex items-center gap-1 mx-auto">
          ← Back to home
        </button>
      </div>
    </div>
