...
export default function Contact({ setPage }: Props) {
  const [isBulk, setIsBulk] = useState(false)
  const [form, setForm] = useState<FormState>({ name: '', email: '', orderSize: '', message: '' })
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
              ...
              <div className="flex items-start gap-3 bg-white border border-navy-600/10 rounded-2xl p-4 cursor-pointer select-none"
                ...
                <div>
                  <p className="text-charcoal-700 font-medium text-[14px] leading-snug">I&apos;m asking about a bulk / party order</p>
                  <p className="text-charcoal-700/45 text-[12px] mt-0.5">50-piece minimums, full cases, and event packages</p>
                </div>
              </div>

              {isBulk && (
                <div className="bg-gold-100 border border-gold-500/20 rounded-xl px-4 py-3 text-[13px] text-charcoal-700/65 leading-relaxed">
                  <span className="font-semibold text-charcoal-700">Bulk orders:</span> We handle events from 50 pieces up to full-case runs. Fill in the form below and tell us your event date and approximate quantity — we&apos;ll come back with a quote within one business day.
                </div>
              )}
        ...
        <div className="flex flex-col gap-5 lg:pt-1">
          ...
          <div className="bg-navy-800 rounded-2xl p-6 border border-gold-500/10">
            ...
            <p className="text-star-white/45 text-[13px] leading-relaxed mb-4">
              50-piece minimums, custom packaging, and white-glove delivery for events. Anaheim delivery is on us.
            </p>
            <button
