...
import type { Page, CartItem, CheckoutFormData } from '../App'

type Props = {
  cart: CartItem[]
  setPage: (p: Page) => void
  placeOrder: (data: CheckoutFormData) => void
}

export default function Checkout({ cart, setPage, placeOrder }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    setTimeout(() => placeOrder({ ...form, payment }), 1000)
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 pt-8 sm:pt-10 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 lg:gap-10">
        <form onSubmit={handleSubmit} className="flex flex-col gap-8" noValidate>
          ...
          <div className="flex flex-col gap-3">
            ...
            <button type="button" onClick={() => setPage('cart')} className="text-sm text-charcoal-700/50 hover:text-charcoal-700 transition-colors text-center">← Back to cart</button>
          </div>
        </form>
        ...
        <div className="lg:sticky lg:top-24 h-fit">
          <div className="bg-white rounded-2xl border border-navy-600/10 p-6">
              ...
              {cart.map((item) => (
                <div key={item.id} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-charcoal-700/40 text-[12px]">× {item.quantity}</p>
                  </div>
            ...
            <div className="border-t border-navy-600/10 pt-4 flex flex-col gap-2">
              ...
              {bulkDiscount > 0 && <div className="flex justify-between text-[14px] text-green-600"><span>Bulk discount</span><span>−${bulkDiscount.toFixed(2)}</span></div>}
